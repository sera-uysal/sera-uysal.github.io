...
> Guidance for AI coding agents working on this site.
> Human-facing overview lives in [README.md](./README.md).

**Live site:** https://sera-uysal.github.io
**Stack at a glance:** Next.js 16 · React 19 · TypeScript · Tailwind v4

## Quick Commands

Everyday commands for local development and pre-commit checks:
...
- Node version pinned in `.nvmrc` (used by CI and local `nvm use`)
- Copy `.env.example` → `.env.local` for local secrets (git-ignored)
- Never commit `.env.local`, `node_modules/`, `out/`, or `.next/`

## Deployment

- Hosted on **GitHub Pages** via GitHub Actions (`.github/workflows/deploy.yml`)
- Merging to `main` triggers an automatic build + deploy
- Static export only (`output: 'export'`)—no server-side runtime
- Analytics: GA4 via `NEXT_PUBLIC_GA_TRACKING_ID` (set as a repo Actions **variable**, never committed)
